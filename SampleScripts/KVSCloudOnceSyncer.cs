 ...
 * ## Sample Notes
 * 
 * To use this in a project, CloudOnce must be installed within it,
 * and KVS should be used instead of PlayerPrefs.
 *
 * Copy/paste the script into your project and it should work automatically,
 * syncing whenever KVS saves or whenever CloudOnce loads.
 *
 * However, the default sync will do NOTHING because the merge is non-destructive.
 * Modify the `DoMerge` function to safely merge things from the cloud into the local KVS data.
 *
 * Enabled for iOS or Android where the CLOUDONCE_GOOGLE define is set.
 */
...
public class KVSCloudOnceSyncer : MonoBehaviour
{
    ...
    bool Disabled => _status == Status.Disabled;
